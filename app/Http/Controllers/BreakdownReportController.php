...
class BreakdownReportController extends Controller
{
    ...
    // ── Storekeeper: see only spare parts breakdown jobs ──────────────────────
    public function storekeeperIndex()
    {
        $reports = BreakdownReport::with(['bus', 'reportedBy'])
            ->where('response_action', 'spare_parts')
            ->whereIn('status', ['approved', 'in_progress', 'resolved'])
            ->latest()
            ->paginate(15);

        return view('breakdowns.storekeeper-index', compact('reports'));
    }
}
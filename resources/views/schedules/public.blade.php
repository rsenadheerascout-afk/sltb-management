...
<body class="bg-gray-50 min-h-screen">
    ...
    <main class="max-w-5xl mx-auto px-6 py-8">
        ...
        <form method="GET" class="flex gap-3 mb-6 flex-wrap">
            ...
            <input type="date" name="date" value="{{ request('date') }}" min="{{ today()->format('Y-m-d') }}"
                class="px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white">
            <button type="submit"
                class="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">Search</button>
            <a href="{{ route('schedules.public') }}"
                class="px-4 py-2 border border-gray-200 text-gray-600 text-sm rounded-lg hover:bg-gray-50">Clear</a>
        </form>
                ...
                <div class="flex items-start justify-between">
                    <div class="flex-1">
                        <div class="flex items-center gap-3 mb-2">
                            ...
                            <span
                                class="text-xs bg-blue-50 text-blue-700 px-2 py-0.5 rounded font-mono">{{ $schedule->bus->depot_reg_no }}</span>
                        </div>
                        ...
                        <div class="flex gap-6 text-sm mt-2">
                            <div>
                                ...
                                <span
                                    class="font-semibold text-gray-800">{{ \Carbon\Carbon::parse($schedule->departure_time)->format('h:i A') }}</span>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <span
                                    class="{{ $available > 5 ? 'text-green-600' : ($available > 0 ? 'text-amber-600' : 'text-red-600') }} font-medium">
                    ...
                    <div class="text-right ml-6">
                        ...
                        @if($available > 0)
                            ...
                            @else
                                <span class="px-5 py-2 bg-gray-100 text-gray-400 text-sm rounded-lg inline-block">
                                    Fully Booked
                                </span>
                            @endif
                        @else
        ...
        @empty
            <div class="bg-white rounded-xl border border-gray-200 p-12 text-center">
                ...
                <a href="{{ route('schedules.public') }}"
                    class="text-blue-600 text-sm hover:underline mt-2 inline-block">View all schedules</a>
            </div>
        @endforelse

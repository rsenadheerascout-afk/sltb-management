<x-dashboard-layout title="Duty Rosters">
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex gap-2">
            {{-- PDF download for currently filtered date --}}
            <a href="{{ route('rosters.pdf', request()->only('date')) }}" class="px-4 py-2 border border-gray-200 text-gray-600 text-sm rounded-lg
                ...
                @forelse($rosters as $roster)
                    <tr class="{{ $roster->duty_date->isToday() ? 'bg-blue-50' : 'hover:bg-gray-50' }}">
                        ...
                        <td class="px-4 py-3">
                            ...
                            @endphp
                            <span
                                class="px-2 py-0.5 rounded text-xs {{ $sc[$roster->status] ?? 'bg-gray-100 text-gray-500' }}">
